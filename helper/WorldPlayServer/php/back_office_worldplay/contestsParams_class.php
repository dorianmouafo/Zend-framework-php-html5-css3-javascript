...
Class contestsparams
	{
	// variables correspondant aux champs de la table contestsparams
	var $id;
	var $uuid;
	var $time_1;
	var $time_2;
	...
	function getContestsparams($id)
		{
		// Requête
		$sql = "SELECT * ";
		$sql .= "FROM contestsparams ";
		$sql .= "WHERE id = '$id';"; 
		// Exécution
		...
		if (mysql_num_rows($rsql) == 0) 
			...
			$this->time_1 = mysql_result($rsql, 0, "time_1");
			$this->time_2 = mysql_result($rsql, 0, "time_2");
			$this->time_3 = mysql_result($rsql, 0, "time_3");
	...
	function getAllContestsparams($whereCritere='', $orderCritere='')
		{
		// Requête
		$sql = "SELECT * ";
		$sql .= "FROM contestsparams ";
	...
	function setContestsparams()
		{
		// Requête
		$sql = "INSERT INTO contestsparams (`uuid`, `time_1`, `time_2`, `time_3`, `points`, `id`) ";
		$sql .= "VALUES (
				'".$this->formatData($this->uuid)."',
				'".$this->formatData($this->time_1)."',
				'".$this->formatData($this->time_2)."',
				'".$this->formatData($this->time_3)."',
	...
	function updateContestsparams($id)
		{
		$sql = "UPDATE contestsparams SET ";
		$sql .= "`uuid` = '".$this->formatData($this->uuid)."', ";
		$sql .= "`time_1` = '".$this->formatData($this->time_1)."', ";
		$sql .= "`time_2` = '".$this->formatData($this->time_2)."', ";
		$sql .= "`time_3` = '".$this->formatData($this->time_3)."', ";
	...
	function deleteContestsparams($id)
		{
		// Requête
		$sql = "DELETE FROM contestsparams WHERE id = '$id';"; 
		// Exécution et résultat	
		if (mysql_query($sql)) 
			{
	...
	function getNewId()
		{
		$sql = mysql_query(" select max(id) from contestsparams ");
