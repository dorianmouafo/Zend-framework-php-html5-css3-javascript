...
<title>Categories - <? echo $dConfig['titreAction']; ?></title>
  ...
  <tr>
    <td height="100%" colspan="3" bgcolor="#FFFFFF"><div align="center">
      <table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0">
        <tr>
          <td><a href="index.php">Administration</a> &gt; Categories </td>
        </tr>
        <tr>
            ...
            <table border="0" cellspacing="0" cellpadding="5">
                <tr valign="top">
                  ...
                  <td><table border="0" cellpadding="5" cellspacing="1" bgcolor="#CCCCCC">
                      <tr>
                        <th bgcolor="#FFFFFF"><div align="left"><strong>Categories &raquo; <? echo $dConfig['titreAction']; ?> </strong></div></th>
                      </tr>
                      <form name="formCategories" onSubmit="return(isDataValid(this))" method="post" action="categories_sql.php?action=<? echo $_REQUEST['action'];?>">
                        <tr>
...
if(($_REQUEST['action']=="Enregistrer")||($_REQUEST['id']))
	...
	if($_REQUEST['id'])
		{
		$newCategories->getCategories($_REQUEST['id']);
		}
                                  ...
                                  <tr>
                                    <td nowrap bgcolor="#FFFFFF"><input type="hidden" name="txt_id" value="<? if ($_REQUEST['id']){echo $_REQUEST['id'];} ?>">
                                        <table width="100%" border="0" cellpadding="3" cellspacing="0">                                           <tr valign="top">
                                            <td nowrap>Uuid</td>
                                            <td><input type="text" name="txt_uuid" maxlength="36" value="<? if ($_REQUEST['id']){echo $newCategories->uuid;} ?>" <? if($_REQUEST['action']=="Supprimer"){?>readonly="1"<? }?>></td>
                                                                                     <tr valign="top">
                                            <td nowrap>Name</td>
                                            <td><input type="text" name="txt_name" maxlength="32" value="<? if ($_REQUEST['id']){echo $newCategories->name;} ?>" <? if($_REQUEST['action']=="Supprimer"){?>readonly="1"<? }?>></td>
                                                                                     <tr valign="top">
                                            <td nowrap>Code</td>
                                            <td><input type="text" name="txt_code" maxlength="3" value="<? if ($_REQUEST['id']){echo $newCategories->code;} ?>" <? if($_REQUEST['action']=="Supprimer"){?>readonly="1"<? }?>></td>
                                                                                </table>
...
?>
                                <table width="100%"  border="0" cellpadding="5" cellspacing="1" bgcolor="#F5F5F5">
                                  <tr bgcolor="#CCCCCC">
                                    <th nowrap><div align="left">N&deg;</div></th>                                     <th nowrap><div align="left">Uuid</div></th>                                     <th nowrap><div align="left">Name</div></th>                                     <th nowrap><div align="left">Code</div></th>                                    <? if($_REQUEST['action']!="Consulter"){?>
                                    <th nowrap><div align="left">Op&eacute;ration</div></th>
                                    <? }?>
                                  </tr>
                                  <?
	$array=$newCategories->getAllCategories('id>0','id');
	...
	?>
                                  <tr valign="top">
                                    <td bgcolor="#FFFFFF"><? echo $i+1;?></td>                                    <td bgcolor="#FFFFFF"><? echo $array[$i]->uuid;?></td>                                    <td bgcolor="#FFFFFF"><? echo $array[$i]->name;?></td>                                    <td bgcolor="#FFFFFF"><? echo $array[$i]->code;?></td>                                    <? if($_REQUEST['action']!="Consulter")
			{?>
