...
<title>Questiontypes - <? echo $dConfig['titreAction']; ?></title>
  ...
  <tr>
    <td height="100%" colspan="3" bgcolor="#FFFFFF"><div align="center">
      <table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0">
        <tr>
          <td><a href="index.php">Administration</a> &gt; Questiontypes </td>
        </tr>
        <tr>
            ...
            <table border="0" cellspacing="0" cellpadding="5">
                <tr valign="top">
                  ...
                  <td><table border="0" cellpadding="5" cellspacing="1" bgcolor="#CCCCCC">
                      <tr>
                        <th bgcolor="#FFFFFF"><div align="left"><strong>Questiontypes &raquo; <? echo $dConfig['titreAction']; ?> </strong></div></th>
                      </tr>
                      <form name="formQuestiontypes" onSubmit="return(isDataValid(this))" method="post" action="questiontypes_sql.php?action=<? echo $_REQUEST['action'];?>">
                        <tr>
...
if(($_REQUEST['action']=="Enregistrer")||($_REQUEST['id']))
	...
	if($_REQUEST['id'])
		{
		$newQuestiontypes->getQuestiontypes($_REQUEST['id']);
		}
                                  ...
                                  <tr>
                                    <td nowrap bgcolor="#FFFFFF"><input type="hidden" name="txt_id" value="<? if ($_REQUEST['id']){echo $_REQUEST['id'];} ?>">
                                        <table width="100%" border="0" cellpadding="3" cellspacing="0">                                           <tr valign="top">
                                            <td nowrap>Uuid</td>
                                            <td><input type="text" name="txt_uuid" maxlength="36" value="<? if ($_REQUEST['id']){echo $newQuestiontypes->uuid;} ?>" <? if($_REQUEST['action']=="Supprimer"){?>readonly="1"<? }?>></td>
                                                                                     <tr valign="top">
                                            <td nowrap>Name</td>
                                            <td><input type="text" name="txt_name" maxlength="32" value="<? if ($_REQUEST['id']){echo $newQuestiontypes->name;} ?>" <? if($_REQUEST['action']=="Supprimer"){?>readonly="1"<? }?>></td>
                                                                                     <tr valign="top">
                                            <td nowrap>Code</td>
                                            <td><input type="text" name="txt_code" maxlength="3" value="<? if ($_REQUEST['id']){echo $newQuestiontypes->code;} ?>" <? if($_REQUEST['action']=="Supprimer"){?>readonly="1"<? }?>></td>
                                                                                </table>
...
?>
                                <table width="100%"  border="0" cellpadding="5" cellspacing="1" bgcolor="#F5F5F5">
                                  ...
                                  <?
	$array=$newQuestiontypes->getAllQuestiontypes('id>0','id');
